...
df_ruchy <- read.csv("dane\\full_moves.csv")
df_dane_partii <- read.csv("dane\\full_game_info.csv")
df_debiuty <- read.csv("dane\\debiuty.csv")
df_debiuty <- df_debiuty %>% filter(name!="")

df_temp <- read.csv("dane\\podsumowanie.csv")
...
ui <- navbarPage(
  ...
  tabPanel(
    ...
    fluidRow(
      ...
      column(6,tableOutput("podsumowanie") )),
  ...
  tabPanel(
    ...
    fluidRow(
      ...
      column(6, plotOutput("rozklad_material"))
      ),
    
    fluidRow(
      column(4,valueBoxOutput("full_win", width=4)),
      column(4,valueBoxOutput("full_loss",width=4)),
      column(4, valueBoxOutput("full_draw",width=4)))
  ),
  ...
  tabPanel(
    ...
    fluidRow(
      column(4,  
             plotOutput("heatmap_ruchy")),
      column(8, plotOutput("debiuty_liczba"))
    ),
    
    fluidRow(
      column(4,uiOutput("select_debiut")),
      column(8, plotlyOutput("animacja",height = "500px"))
      )
  )
)
